...
const CreditAccountsList: React.FC<{
  ...
  selectedAccount: CreditAccount | undefined;
  setSelectedAccount: (account: CreditAccount) => void;
  ...
  const getTotalBalance: () => number = (): number => {
    return accounts
      .map((account: CreditAccount): number => account.balance)
      .reduce((a, b) => a + b, 0);
  };
  ...
  const renderAccountsList: () => React.ReactElement = (): React.ReactElement => {
    return (
      <ul role="list" className="divide-y divide-gray-100">
        {accounts.map(
          (account: CreditAccount): React.ReactElement => (
            <li
    ...
    <>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-x-4">
          ...
          <PlusCircleIcon
            onClick={(): void => console.log('Create Credit Account')}
            aria-hidden="true"
