...
const API_NAME: string = 'UpdateTransaction';
...
export default async function handler(
  ...
  try {
    const backendResponse: AxiosResponse = await WalterBackend.updateTransaction(
      apiUrl,
      apiKey,
      accessToken,
      request.body.transactionDate,
      request.body.transactionId,
      request.body.updatedMerchantName,
      request.body.updatedTransactionCategory
    );
    ...
    if (backendResponse.status === HttpStatus.UNAUTHORIZED) {
      ...
      const backendRetryResponse: AxiosResponse = await WalterBackend.updateTransaction(
        apiUrl,
        apiKey,
        newAccessToken,
        request.body.transactionDate,
